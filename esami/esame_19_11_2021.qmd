...
1. (1pt) Si creino i vettori `x_river` e `x_no_river` contenenti i valori della variabile `crim`, rispettivamente per le osservazioni vicine al fiume (`chas` = 1) e lontane dal fiume (`chas` = 0). 

2. (4pt) Siano $x_1,\dots,x_n$ delle osservazioni numeriche. La _differenza semplice media_ è un indicatore di variabilità definito come segue 
$$
...
1. (4pt) Sia $x_{(1)}, \dots, x_{(n)}$ il campione ordinato dei dati $x_1,\dots,x_n$ e sia $\bar{x}$ la media aritmetica. La _differenza semplice media_ ammette una rappresentazione alternativa, pari a:
$$
...
$$
dove $U_1$ ed $U_2$ sono due variabile aleatorie _uniformi_ in $(0,1)$ ed indipendenti tra loro. 

1. (2pt) Si scriva in **R** la funzione `r_log_sin(R)` che simula `R` valori pseudo-casuali distribuiti come la variabile  $X$.

1. (2pt) Si fornisca un'approssimazione delle quantità $E(X)$ e $E(X^2)$. 

1. (2pt) Si quantifichi l'errore commesso al punto precedente.

1. (2pt) Si approssimino le probabilità $P(X \le x)$, per $x = -1, 0, 1$. 

1. (2pt) Si ottenga una stima della densità di $X$. Sulla base di quest'ultima stima e di tutti gli indicatori precedenti, a cosa "assomiglia" la distribuzione di $X$?
...
1. (3pt) Si considerino le osservazioni `10, 3, 12, 20, 32, 8`. Si disegni la funzione di log-verosimiglianza corrispondente nell'intervallo $(0, 30)$. 

1. (3pt) Utilizzando opportuni strumenti di approssimazione numerica si calcoli la stima di massima verosimiglianza $\hat{\theta}$. 

1. (2pt) Si calcoli il valore della log-verosimiglianza nel punto di massimo e la si confronti con il valore $\ell(10 + \pi/4)$. Si commentino i risultati.
...
3.  Siano $X_1,\dots,X_n$ dei valori iid da una Cauchy standard e si calcoli $$
...
2.  Si verifichi tramite simulazione la disugaglianza di Markov nel caso $g(x) = e^{x/100}$ e con $X \sim \text{Ga}(10,1/10)$, per $k = 1,2,3$.
...
3.  Si scriva la funzione `rsn(n, alpha)` che simula $n$ valori pseudo-casuali da una funzione normale asimmetrica di parametro $\alpha$ sfruttando la seguente relazione per $X$ $$

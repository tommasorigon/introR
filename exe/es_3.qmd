...
La distribuzione beta-binomiale di parametri $\alpha, \beta > 0$ ed
$n \in \mathbb{N}$ di è una variabile aleatoria discreta $X$ è tale che
...
1.  Si verifichi tramite simulazione la disugaglianza di Markov nel caso
    $g(x) = x^2$ e con $X \sim \text{N}(0,1)$, per $k = 1,2,3$. Si
    confronti il risultato con i valori teorici.

2.  Si verifichi tramite simulazione la disugaglianza di Markov nel caso

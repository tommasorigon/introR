...
```{r}
# Crucial command to make the images appear online
knitr::opts_knit$set(base.url='https://github.com/tommasorigon/introR/blob/master/lezioni/')
```
...
\item Il simbolo `NULL` significa **non definito**, perché non esiste la nozione di dimensione per un generico vettore **R**.
...
Alcuni \empb{comandi di base} per manipolare le matrici sono i seguenti
